...
import threading
import socket
from backend_llamacpp import LlamaCppBackend, LlamaCppConfig
...
def build_ui() -> gr.Blocks:
    backend = LlamaCppBackend(LlamaCppConfig(base_url="http://127.0.0.1:5001"))
    gse=Gscript_editer()
    ...
    with gr.Blocks(title="Easy Novel Assistant osuChitsu") as demo:
                ...
                with gr.Tabs():
                    ...
                    with gr.TabItem("llama.cpp"):
                        ...
                        llama_server_exe = gr.Textbox(
                            label="llama-server 実行ファイルパス（起動する場合のみ）",
                            placeholder="例: llama-server  または  /path/to/llama-server",
                            value="./llama-server_ubuntu",
                        )
        ...
        def on_retry_stream(
            ...
            try:
                for delta in backend.generate_stream(prompt, params):
                    acc += delta
        ...
        def on_start(exe: str, model: str, layers: int, base_url: str, context_length: int):
            ...
            if not exe.strip():
                yield {status:"llama-server を外部で起動済みなら exe は空でOKです。base_url だけ合わせてください。"}
            ...
            try:
                ...
                msg = backend.start(exe.strip(), model, layers=layers, host=host, port=port, context_length=context_length)
        ...
        def on_exit():
            ...
            3) プロセス強制終了（サーバごと落ちる）
            """
            # 先に llama-server を止める（backend.stop() を使う）
            try:
                backend.stop()
            except Exception:
                pass
        ...
        def import_json(path:str):
            if os.path.exists(path):
                with open(path,mode="r",encoding="utf-8")as f:
                    datas=json.load(f)
                    param=datas["params"]
                    if "koboldcpp" in datas:
                        kobo=datas["koboldcpp"]
                    else:
                        kobo=datas["llama_cpp"]
                    dolist=datas["dolist"]
        ...
        start_btn.click(on_download,inputs=[model_choice],outputs=[status]).then(on_start, inputs=[llama_server_exe, model_choice, layers, base_url, context_length], outputs=[status])
